  ...
  async onPair(session) {
    ...
    session.setHandler('login', async ({ username, password }) => {
      try {
        ...
        // Show friendly message — raw API error contains JSON which confuses users
        const friendly = err.message.includes('invalid_grant') || err.message.includes('401')
          ? 'Incorrect email or password. Please check your Watts Vision+ credentials at smarthome.wattselectronics.com'
  ...
  async onRepair(session, device) {
    session.setHandler('login', async ({ username, password }) => {
      try {
        const api = new WattsApi(this.log.bind(this), this.error.bind(this));
        await api.authenticate(username, password);
        await device.setStoreValue('credentials', { username, password });
        await device.reinitApi();
        this.log('Re-auth successful for:', device.getName());
        return true;
      } catch (err) {
        this.error('Re-auth failed:', err.message);
        const friendly2 = err.message.includes('invalid_grant') || err.message.includes('401')
          ? 'Incorrect email or password. Please check your Watts Vision+ credentials at smarthome.wattselectronics.com'
          : 'Login failed: ' + err.message;
        throw new Error(friendly2);
      }
    });
  }
}
